...
namespace GestionPlazasVacantes.Services
{
    public static class DbInitializer
    {
        public static void Initialize(AppDbContext context)
        {
            ...
            // Si hay usuarios y plazas, asumimos que la BD está inicializada
            if (existenUsuarios && existenPlazas)
            {
                return;
            }
            ...
            if (!existenPlazas)
            {
                plazas = new List<PlazaVacante>
                {
                    new PlazaVacante
                    {
                        ...
                        FechaCierre = null,
                        UsuarioAsignadoId = usuarios.Count > 1 ? usuarios[1].Id : (int?)null
                    },
                    new PlazaVacante
                    {
                        ...
                        EstadoFinal = "En Proceso",
                        FechaCierre = null,
                        UsuarioAsignadoId = usuarios.Count > 2 ? usuarios[2].Id : (int?)null
                    },
                    new PlazaVacante
                    {
                        TipoConcurso = "Interno",
            ...
            {
                plazas = context.PlazasVacantes.ToList();
            }

            // ===== CREAR POSTULANTES (si no existen) =====
            if (!context.Postulantes.Any())
            {
                 ...
                 var seguimientos = new List<SeguimientoPostulante>
                {
                    ...
                    new SeguimientoPostulante
                    {
                        ...
                        Observaciones = "Excelente desempeño en prueba técnica.",
